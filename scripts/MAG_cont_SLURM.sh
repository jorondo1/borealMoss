...
if [[ ! -f $OUT/${SAMPLE}/aligned_paired.1.fastq.gz ]] || [[ ! -f $OUT/${SAMPLE}/aligned_paired.2.fastq.gz ]]; then
	...
	--threads $SLURM_NTASKS -S $OUT/${SAMPLE}/$SAMPLE.sam 
...
if [[ ! -f $OUT/${SAMPLE}/aligned.${MAG}.1.fastq.gz ]] || [[ ! -f $OUT/$SAMPLE/aligned.${MAG}.2.fastq.gz ]]; then
	...
	bowtie2 --very-sensitive --no-unal -x $HOST_IDX \
	-1 $OUT/$SAMPLE/aligned_paired.1.fastq.gz -2 $OUT/$SAMPLE/aligned_paired.2.fastq.gz \
 	--al-conc-gz $OUT/$SAMPLE/aligned.${MAG}.%.fastq.gz --threads $SLURM_NTASKS -S $OUT/$SAMPLE/${SAMPLE}_${MAG}.sam
